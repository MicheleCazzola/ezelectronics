...
class ProductDAO {
    ...
    createProduct(model: string, category: string, quantity: number, details: string | null, sellingPrice: number, arrivalDate: string | null): Promise<void> {
        return new Promise<void>((resolve, reject) => {
            try {
                const sql = "INSERT INTO product_descriptor(Model, SellingPrice, ArrivalDate, AvailableQuantity, Category, Details) VALUES (?,?,?,?,?,?)"
                db.run(sql, [model, sellingPrice, arrivalDate, quantity, category, details], (err: Error | null) => {
                    if (err) {
                        if (err.message.includes("UNIQUE constraint failed: product_descriptor.Model")) reject (new ProductAlreadyExistsError)
                        reject(err)
                    }
                    resolve()
                })
     ...
     */
    increaseQuantity(model: string, newQuantity: number, changeDate: string | null): Promise<number> {
        return new Promise<number>((resolve, reject) => {
            try {
                const sql = "SELECT AvailableQuantity FROM product_descriptor WHERE Model = ?"
                db.get(sql, [model], (err: Error | null, row: any) => {
                    if (err) reject(err)
                    ...
                    let param;
                    if (changeDate != null) {
                        sqlUpdate = "UPDATE product_descriptor SET AvailableQuantity = ?, ArrivalDate = ? WHERE Model = ?"
                        param = [qt, changeDate, model];
                    } else {
                        sqlUpdate = "UPDATE product_descriptor SET AvailableQuantity = ? WHERE Model = ?"
                        param = [qt, model];
                    }
     ...
     * @returns A Promise that resolves to the new available quantity of the product.
     */
    decreaseQuantity(model: string, quantity: number, sellingDate: string | null): Promise<number> {
        return new Promise<number>((resolve, reject) => {
            try {
                const sql = "SELECT AvailableQuantity FROM product_descriptor WHERE Model = ?"
                db.get(sql, [model], (err: Error | null, row: any) => {
     ...
     * @returns A Promise that resolves to an array of Product objects.
     */
    getAllAvailableProducts(grouping: string | null, category: string | null, model: string | null): Promise<Product[]> {
        return new Promise<Product[]>((resolve, reject) => {
            try {
                let sql;
                let param;
                if (grouping == "category") {
                    sql = "SELECT * FROM product_descriptor WHERE Category = ? AND AvailableQuantity > 0"
                    param = [category];
                } else if (grouping == "model") {
                    sql = "SELECT * FROM product_descriptor WHERE Model = ? AND AvailableQuantity > 0"
                    param = [model];
                } else {
